...
from ..resources import OAIOAIRadiogenomicSampler, PRE_STAGED_DIR, STAGED_DIR, OAI_RADIOGENOMICS_COLLECTION_NAME

log = get_dagster_logger()

@asset()
def oai_nsclc_radiogenomic_samples(oai_nsclc_radiogenomic_sampler: OAIOAIRadiogenomicSampler) -> pl.DataFrame:
    """
    OAI OAI Radiogenomic Samples. Samples are placed in data/pre-staged/nsclc-radiogenomics-samples.
    """
    return oai_nsclc_radiogenomic_sampler.get_samples()


@asset()
def staged_oai_nsclc_radiogenomic_samples(oai_nsclc_radiogenomic_samples) -> None:
    """
    Staged OAI OAI Radiogenomic Samples. Samples are placed in data/staged/nsclc-radiogenomics-samples.
    """

    for row in oai_nsclc_radiogenomic_samples.iter_rows(named=True):
        patient_id = row['Patient ID']
        patient_pre_staged_path = PRE_STAGED_DIR / OAI_RADIOGENOMICS_COLLECTION_NAME / patient_id
        ...
        for study in studies:
            ...
            for series_id, ds in series.iterrows():
                patient_staged_path = STAGED_DIR / OAI_RADIOGENOMICS_COLLECTION_NAME / "dagster" / patient_id / ds.StudyInstanceUID
